...
当前版本让更多开发环节留在同一个工作区：可应用内预览本地或远程 Web 项目、双栏并排打开文件、查看图片和播放支持的媒体，并在跨面板操作时持续看到 Agent 的运行状态。同时也改善了 SSH 连接复用、移动端终端输入、Git 刷新与 Diff 渲染、长时间使用的资源占用和界面一致性。完整细节请查看 [1.1.5 更新说明](/zh/releases/1.1.5)。
